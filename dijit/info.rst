...
To learn more about themes, see :ref:`Dijit themes <dijit-themes>`.
...
The simplest way to access a widget is :ref:`dijit.byId <dijit/byId>`. When the widget is created, if the Node used to create the widget (eg: srcNodeRef) had a DOM attribute ``id``, that becomes the widget's id in the :ref:`dijit.regsitry <dijit/registry>`.
...
It also supports a hash API like :ref:`dojo.attr() <dojo/attr>`, for setting multiple attributes:
